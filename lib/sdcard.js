...
function getRemountPath (drive, cb) {
  ...
  if (process.platform === 'darwin' || process.platform === 'win32') {
    getRemountPathMacWindows(drive, 0, 0, false, cb)
  } else if (process.platform === 'linux' ||
...
function getRemountPathMacWindows (drive, remountCount, checkCount, mounted, cb) {
  regetDrive(drive, function (error, drive) {
    ...
    if (checkCount >= drive.mountpoints.length) {
      setTimeout(function() {
        getRemountPathMacWindows(drive, remountCount + 1, 0, false, cb)
      }, 5000)
      return
    }
    ...
    /*if (drive.mountpoints.length == 0) {
      setTimeout(function() {
        getRemountPathMacWindows(drive, remountCount + 1, false, cb)
      }, 5000)
      return
    }*/
    ...
    fs.stat(bootcodefile, function (error, stats) {
      if (error) {
        ...
        setTimeout(function() {
          getRemountPathMacWindows(drive, remountCount, checkCount + 1, true, cb)
        }, 5000)
        return
      }
...
function checkMountpointsUnix (drive, remountCount, checkCount, cb) {
  ...
  if (checkCount >= drive.mountpoints.length) {
    setTimeout(function() {
      getRemountPathUnix(drive, remountCount + 1, 0, cb)
    }, 5000)
    return
  }
